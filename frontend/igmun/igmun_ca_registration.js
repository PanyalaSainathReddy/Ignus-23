...
ca_register_form.addEventListener('submit', function(e){
  ...
  fetch('https://api.ignus.co.in/api/igmun/precaregform/', {
    method: 'POST',
    body: JSON.stringify({
        full_name:full_name,
        email:email,
    ...
    .then(function(response){
        if(response.status==201){
            sessionStorage.setItem("showmsg", "Successfully Registered as Igmun CA!");
            window.location.replace("/igmun/index.html");
        }
        else{
            alert("This Email has already been registered for Igmun CA!");
        }
    })
